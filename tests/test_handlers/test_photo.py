...
async def test_photo():
    ...
    test_message.photo[-1].file_id = "test_file_id"
    ...
    with patch('handlers.photo.Message', return_value=test_message), \
         patch('handlers.photo.bot', test_bot), \
         patch('handlers.photo.vision', test_vision), \
         patch('handlers.photo.ocr', test_ocr), \
         patch('handlers.photo.chat', test_chat):
        # Вызываем функцию photo
        await photo(test_message)
